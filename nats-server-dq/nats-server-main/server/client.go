...
func (c *client) deliverMsg(prodIsMQTT bool, sub *subscription, acc *Account, subject, reply, mh, msg []byte, gwrply bool) bool {
	...
	if hasHeader && canReceiveHeader {
		...
		hv := bytes.Split(msg, []byte(_CRLF_))
		for _, s := range hv {
			kv := bytes.Split(s, []byte(":"))
			if len(kv) > 1 {
				extractCarrier.Set(string(kv[0]), string(kv[1]))
			}
		}

		propagator := propagation.NewCompositeTextMapPropagator(propagation.TraceContext{}, propagation.Baggage{})

		parentCtx := propagator.Extract(context.Background(), extractCarrier)
		injectContext, span := otel.Tracer("opentelemetry-event-tracing").Start(parentCtx,
			"NATS.MSG.DELIVER (NATS Broker)")

		injectCarrier := propagation.MapCarrier{}
		propagator.Inject(injectContext, injectCarrier)

		var dl = 0
		var newMsg []byte
		for i, s := range hv {
			kv := bytes.Split(s, []byte(":"))
			if len(kv) > 1 && len(injectCarrier.Get(string(kv[0]))) > 0 {

				fmt.Printf("Difference in length of trace context, Original=%d, New=%d \n",
					len(extractCarrier.Get(string(kv[0]))), len(injectCarrier.Get(string(kv[0]))))

				dl = dl + (len(extractCarrier.Get(string(kv[0]))) - len(injectCarrier.Get(string(kv[0]))))

				newMsg = append(newMsg, kv[0]...)
				newMsg = append(newMsg, []byte(":")...)
				newMsg = append(newMsg, []byte(injectCarrier.Get(string(kv[0])))...)
				newMsg = append(newMsg, []byte(_CRLF_)...)
			} else {
		...
		oldszb, _ := strconv.Atoi(string(c.pa.szb))
		oldhdb, _ := strconv.Atoi(string(c.pa.hdb))

		var newszb = oldszb - dl
		var newhdb = oldhdb - dl

		c.pa.szb = []byte(strconv.Itoa(newszb))
		c.pa.hdb = []byte(strconv.Itoa(newhdb))

		mh = c.msgHeader(subject, reply, sub)
		msg = newMsg
